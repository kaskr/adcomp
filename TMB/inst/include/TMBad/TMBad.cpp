...
std::ostream &operator<<(std::ostream &os, period &x) {
  os << "begin: " << x.begin;
  os << " size: " << x.size;
  os << " rep: " << x.rep;
  return os;
...
void StackOp::print(global::print_config cfg) {
  ...
  std::cout << cfg.prefix << " opstack = " << tmp << "\n";
...
void global::print(print_config cfg) {
  ...
  std::cout << cfg.prefix;
  std::cout << setw(7) << "OpName:" << setw(7 + have_subgraph)
            << "Node:" << setw(13) << "Value:" << setw(13)
            << "Deriv:" << setw(13) << "Index:";
  std::cout << "    "
            << "Inputs:";
  std::cout << endl;
  for (size_t i = 0; i < opstack.size(); i++) {
    std::cout << cfg.prefix;
    std::cout << setw(7) << opstack[i]->op_name();
    if (have_subgraph) {
      if (sgm[i])
        std::cout << cfg.mark;
      else
    ...
    for (int j = 0; j < numvar + (numvar == 0); j++) {
      ...
      if (j == 0) {
        ...
        for (int k = 0; k < ninput; k++) {
          if (k == 0) std::cout << "   ";
          std::cout << " " << inputs[ptr_old.first + k];
        }
      }
      std::cout << endl;
      if (numvar > 0) {
...
void remap_identical_sub_expressions(global &glob, bool all_allow_remap) {
  ...
  if (reject > 0) {
    std::cout << "Rejected remappings: " << reject << " of " << total << "\n";
  }
...
void reorder_graph(global &glob, std::vector<Index> inv_idx,
  ...
  if (true) {
    ...
    std::cout << "marked proportion:" << (double)c / (double)marks.size()
              << "\n";
  }
