...
template <bool pos>
struct IncOp : global::Operator<2, 0> {
  static const bool is_linear = true;
  static const bool forward_updating = true;
  ...
  void operator()(ad_aug &x, const ad_aug &y) {
    ...
    global::Complete<IncOp> *pOp = glob->template getOperator<IncOp>();
    glob->add_to_opstack(pOp);
    pos ? glob->values[i0] += glob->values[i1]
...
inline ad_aug &UpdatingAccess<ad_aug>::operator+=(const ad_aug &other) {
  if (this->x.updatable() && other.ontape()) {
    IncOp<true>()(this->x, other);
  } else {
    this->x += other;
  }
  return this->x;
