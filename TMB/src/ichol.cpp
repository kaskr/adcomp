...
/*
  ...
  This routine locates strucural orthogonal rows, and summarizes the
  result in a sparse matrix referred to as the 'parallel schedule'.

  Each column of the parallel schedule defines a 'batch' (j) of row
  'breaks' i[p[j]:p[j+1]] such that the corresponding row intervals
  (chunks) are mutually structural orthogonal.

  NOTE: This is only useful after permuting the A matrix (otherwise
  there's usually no structural independence).
*/
...
struct thread_data {
  ...
  double err;
  thread_data() {}
  thread_data(size_t n) : mark(n, 0), x(n, 0), err(0) {}
};
void cs_ichol_update_omp_wrk (const cs *A, cs *L, double* err,
  ...
  double d, lki, *Lx, *Cx ;
  csi top, i, p, k, *Li, *Lp, *Ri, *Rp, *Cp, *Ci ;
  cs *C;
  C = (cs*) A;
  Cp = C->p ; Ci = C->i ; Cx = C->x ;
  Lp = L->p ; Li = L->i ; Lx = L->x ;
...
#pragma omp parallel for
    ...
    if (err) {
      for (size_t i=0; i<tdat.size(); i++)
        *err = std::max(*err, tdat[i].err);
    }
    return true;
}

// =================== PARALLEL ADJOINT CODE

struct thread_data_adjoint {
  std::vector<int> mark;
  std::vector<double> dx;
  thread_data_adjoint() {}
  thread_data_adjoint(size_t n) : mark(n, 0), dx(n, 0) {}
};
void cs_ichol_adjoint_update_omp_wrk (cs *L,
                                      const cs *R, int K, int N,
                                      std::vector<int> &c,
                                      std::vector<double> &dL,
                                      thread_data_adjoint &tdat
                                      ) {
  std::vector<double> &dx = tdat.dx;
  std::vector<int> &mark = tdat.mark;
  double *Lx ;
  csi top, i, p, k, *Li, *Lp, *Ri, *Rp ;
  Lp = L->p ; Li = L->i ; Lx = L->x ;
  Rp = R->p ; Ri = R->i ;
  // LDL adjoint
  for (k = K+N ; k > K ; ) {
...
void cs_ichol_adjoint_update_omp (cs *L, cs *S) {
  double *Lx ;
  csi p, k, n, *Lp, *Sp, *Si ;
  n = L->n ;
  std::vector<int> c(n);
...
SEXP tmb_ichol_adjoint_update(SEXP L) {
  ...
  cs A = r2cs(X);
  SEXP SCH = Rf_getAttrib(L, Rf_install("parallel_schedule"));
  if (!Rf_isNull(SCH)) {
    cs S = r2cs(SCH);
    cs_ichol_adjoint_update_omp(&A, &S);
  } else {
