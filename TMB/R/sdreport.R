...
sdreport <- function(obj,par.fixed=NULL,hessian.fixed=NULL,getJointPrecision=FALSE,bias.correct=FALSE,
  ...
  if(!is.null(r)){
    ...
    if (intern) {
        ...
        r <- which(rep( names(obj$env$parameters), sapply(obj$env$parameters, length) ) %in% obj$env$.random)
    }
...
TMB_epsilon <- function(data, moment=1, scale=1, what = c("reportvector", "theta"),
    ...
    data$TMB_epsilon_which_ <- as.numeric(which)
    data
}
...
sdreport_intern <- function(obj,
    ...
    if (what == "reportvector") {
        ...
        random <- which(rep( names(parameters), sapply(parameters, length) ) %in% obj$env$.random)
        which <- random
        n <- length(random)
    }
    ## For epsilon method
