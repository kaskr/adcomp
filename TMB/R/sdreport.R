...
##' by
##' \deqn{V \begin{pmatrix}{ \hat u - u \cr \hat\theta - \theta }\end{pmatrix} \approx
##' \begin{pmatrix}{ H_{uu}^{-1} & 0 \cr 0 & 0 }\end{pmatrix} +
##' J V(\hat\theta) J'
##' }
##' where \eqn{H_{uu}} denotes random effect block of the full joint
##' Hessian of \code{obj$env$f} and \eqn{J} denotes the Jacobian of
##' \eqn{\begin{pmatrix}{\hat u(\theta) \cr \theta}\end{pmatrix}} wrt. \eqn{\theta}.
...
##' parameters \eqn{\theta}:
##' \deqn{V\left(\phi(\hat u,\hat\theta) - \phi(u,\theta) \right)\approx \nabla\phi V \begin{pmatrix}{
##' \hat u - u \cr \hat\theta - \theta }\end{pmatrix}\nabla\phi'}
##'
...
##' \begin{pmatrix}{ \hat u - u \cr \hat\theta - \theta }\end{pmatrix} ^{-1} } will be part of the
