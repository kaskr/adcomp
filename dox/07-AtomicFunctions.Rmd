...
Custom functions and derivatives can be added to the TMB library. This may be necessary due to the following reasons:
...
TMB uses CppAD as engine for reverse mode derivatives. In order to add a new primitive function

$$f: R^n \rightarrow R^m$$

we must inform CppAD how to calclualate derivatives of this function in reverse mode. That is, for any range space vector $w \in R^m$ we must calculate the gradient of the function $R^n \rightarrow R$ given by
...
When coding up reverse mode derivatives we can assume that the function value $W(x)$ has already been computed during a forward pass. For efficiency reasons we should use this intermediate calculation rather than re-calculating $W(x)$ in the reverse pass.
...
Let's explain in details what is going on. The macro takes four arguments:
...
4. `ATOMIC_REVERSE`: How to calculate the reverse mode derivatives for a general `Type`. Again `tx` and `ty` denote function input and output but now `ty` has been computed and is available as an intermediate value. The vectors `px` and `py` denote partial derivatives of the end result wrt. $x$ and $y$ respectively. `py` is given and we must calculate `px` using the chain rule. This first order derivative rule is automatically expanded up to higher orders required when using TMB's random effects calculations.
...
The complete example using Newton's method to calculate the Lambert W function is:
...
For the Lambert W function we knew how to calculate the derivatives. There are cases for which the derivatives are impossible (or difficult) to write down. If this is the situation you're in you may want to try using forward mode AD to help defining an atomic function. A full worked out example is available here: `r doxylink("adaptive_integration.cpp")`. Derivatives are calculated automatically and if-else branching is allowed. The main downside with the approach is that it is limited to functions with very few inputs.

A second technique worth mentioning is checkpointing. It is demonstrated in the example `r doxylink("register_atomic.cpp")`. It does not work for adaptive algorithms but is otherwise automatic. It is useful to reduce AD memory usage in cases where the same operation sequence is being applied many times.
