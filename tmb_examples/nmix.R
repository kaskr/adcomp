...
# There is no direct equivalent of "phases" in TMB, but it can be
# achieved via the "map" argument to MakeADFun. "phases" means keeping
# a subset of the parameters fixed when the model is first fit.

## Phase 1: u and log_sigma fixed
map <- list(u=factor(rep(NA,data$nG)),                 # Say which parameters are fixed
            log_sigma=factor(NA))
